...
export function usePwa(): PwaData {
  ...
  const { value: canInstallprompt, setTrue: onCanInstallprompt } = useBoolean(false)
  ...
  const handleAppinstalled = useCallback(() => onAppinstalled(), [onAppinstalled])
  const handleBeforeinstallprompt = useCallback<(event: BeforeInstallPromptEvent) => void>(
    (event) => {
  ...
  const isLoading = useMemo(() => Object.values(completed).includes(false), [completed])
  ...
  useEffect(() => {
    try {
      ...
      if (!lownerUserAgent.includes('iphone') && !lownerUserAgent.includes('ipad')) {
        return
      }
