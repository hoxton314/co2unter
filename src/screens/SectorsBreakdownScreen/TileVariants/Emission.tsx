...
import { TileBox } from '../SectorsBreakdownScreen.styles'
...
export const Emission: FC<TileSwapperProps> = observer(({ sectorName, sectorType }) => {
  ...
  return (
    <TileBox>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" height="80px" viewBox="0 -960 960 960" width="80px" fill="#4b4b4b">
      ...
      {`${totalEmissions}: ${Math.floor((sectorType !== 'user' ? store.AppState.sectors[sectorType]?.totalEmissions.totalEmissions : store.AppState.calculatedEmission.totalEmissions) as number)}`}
    </TileBox>
  )
})
