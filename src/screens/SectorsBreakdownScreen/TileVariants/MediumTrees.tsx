...
export const MediumTrees: FC<TileSwapperProps> = observer(({ sectorName, sectorType }) => {
  ...
  return (
    <TileBox>
      ...
      {`${mediumTreeAbsorption}: ${Math.floor((sectorType !== 'user' && store.AppState.sectors[sectorType]?.totalEmissions.mediumTreeAbsorption) as number)}`}
    </TileBox>
  )
})
