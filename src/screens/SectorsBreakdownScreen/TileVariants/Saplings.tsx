...
export const Saplings: FC<TileSwapperProps> = observer(({ sectorName, sectorType }) => {
  ...
  return (
    <TileBox>
      ...
      {`${seedlingAbsorption}: ${Math.floor((sectorType !== 'user' ? store.AppState.sectors[sectorType]?.totalEmissions.seedlingAbsorption : store.AppState.calculatedEmission.seedlingAbsorption) as number)}`}
    </TileBox>
  )
})
