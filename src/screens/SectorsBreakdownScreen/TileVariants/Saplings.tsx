...
export const Saplings: FC<SaplingsProps> = observer(({ sectorData, sectorName }) => {
  const store = useContext(StoreContext)
  const { smallTreeAbsorption } = store.TranslationsState.translations.CalculatedEmissions
  return (
    <>
      {sectorName}
      ...
      {`${smallTreeAbsorption} ${sectorData.seedlingAbsorption}`}
    </>
  )
})
