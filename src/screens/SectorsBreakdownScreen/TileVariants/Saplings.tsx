...
export const Saplings: FC<TileSwapperProps> = observer(({ sectorName, sectorType }) => {
  ...
  return (
    <>
      ...
      {`${seedlingAbsorption} ${sectorType !== 'user' ? store.AppState.sectors[sectorType]?.totalEmissions.seedlingAbsorption : store.AppState.calculatedEmission.seedlingAbsorption}`}
    </>
  )
})
